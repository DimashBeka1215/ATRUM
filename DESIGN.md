...
> **Glass mode применяется ТОЛЬКО на экранах чата** (переписка поверх обоев). Это тема чатов, а не
> приложения. На всех остальных экранах (профиль, настройки, join chat, списки) — обычные непрозрачные
> токены и карточки, glass-drawables там не используются.
