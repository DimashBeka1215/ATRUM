...
class NostrTransport(
    ...
    /**
     * Публикация файла/чанка с НАСТОЙЧИВЫМ ретраем (до [FILE_PUBLISH_MAX_ATTEMPTS] попыток
     * с растущей паузой). Фото/голосовые — это МНОГО последовательных событий (чанки +
     * хеши + манифест); один случайный сбой (нестабильная сеть, DPI-помеха на RTT, реле
     * забанило burst) не должен ронять всю заливку, если остальные события уже ушли.
     *
     * ⚠️ ПО ПРОСЬБЕ ПОЛЬЗОВАТЕЛЯ: раньше был ровно 1 повтор — этого хватало для случайного
     * сбоя, но НЕ хватало против сети, которая режет пакеты пачками (см. NOSTR_CHUNK_CHARS
     * выше) — там несколько попыток подряд могут закономерно проваливаться, и нужен запас
     * попыток, а не один быстрый повтор. Приоритет — «дошло и собралось», а не скорость
     * (пользователь явно согласился на более долгую отправку). Растущая пауза — не долбить
     * реле чаще во время помехи, дать сети/DPI-окну «отпустить».
     *
     * Не бесконечный цикл — это НЕ замена scheduleRebroadcast/ретраю всего сообщения на
    ...
    private suspend fun publishFileWithRetry(name: String, content: String) {
        ...
        for (attempt in 0 until FILE_PUBLISH_MAX_ATTEMPTS) {
            try {
                ...
                lastError = e
                if (attempt < FILE_PUBLISH_MAX_ATTEMPTS - 1) {
                    delay(FILE_PUBLISH_BACKOFF_MS[attempt])
                }
            }
        }
        throw lastError ?: RuntimeException("publishFileWithRetry: исчерпаны попытки для $name")
    }
    ...
    override suspend fun saveFileChunked(
        ...
        chunks.forEachIndexed { i, chunk ->
            ...
            // чанка — там ниже своя пауза перед хешами/манифестом, дублировать незачем.
    ...
    companion object {
         ...
         *  сама пауза тоже не должна быть узнаваемым паттерном. */
        private const val CHUNK_JITTER_MIN_MS = 250L
        private const val CHUNK_JITTER_MAX_MS = 700L

        /** Сколько раз пытаться опубликовать ОДИН чанк/файл, прежде чем сдаться (см.
         *  publishFileWithRetry) — надёжность важнее скорости по прямой просьбе пользователя. */
        private const val FILE_PUBLISH_MAX_ATTEMPTS = 6
        /** Растущая пауза между попытками (мс), последний элемент — после предпоследней попытки. */
        private val FILE_PUBLISH_BACKOFF_MS = longArrayOf(500L, 1_000L, 2_000L, 4_000L, 8_000L)
