...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Версия 23: флаг верификации собеседника (partnerVerified) для галочки у ника в
         * списке чатов. Колонка с дефолтом 0 — старые 1:1-чаты = не верифицирован (пока
         * фоновый опрос не пересчитает криптографически, см. VerifiedBadge), не затронуты.
         */
        private val MIGRATION_22_23 = object : Migration(22, 23) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE chats ADD COLUMN partnerVerified INTEGER NOT NULL DEFAULT 0")
            }
        }

        /**
         * Версия 25: «устойчивая доставка медиа» — флаг ЧАТА (см. Chat.resilientMedia).
         * Аддитивно: новая колонка с дефолтом 0 (выключено), существующие чаты не меняются
         * и продолжают работать в прежнем быстром режиме (§17).
         */
        ...
        /**
         * Версия 24: закреплённый identity-ключ участника (ADR_MESSAGE_AUTHENTICITY.md, Фаза 0) —
         * фундамент неподделываемого авторства. Аддитивно: новое NULL-поле, старые строки целы.
         */
        private val MIGRATION_23_24 = object : Migration(23, 24) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE chat_participants ADD COLUMN pinnedIdentityPubKey TEXT")
            }
        }
