...
data class Message(
    ...
    /**
     * Абсолютный индекс СТРОКИ этого сообщения в chat.txt (0-based, включая нерасшифрованные/
     * забаненные строки) — для корректной галочки «прочитано». Сравнивается с partnerLastReadIndex
     * (= число строк chat.txt на момент прочтения собеседником), а НЕ позиция в адаптере: адаптер
     * скрывает нерасшифрованные (mapNotNull) и забаненных (withoutBanned) + добавляет pending/
     * системные → позиция ≠ строка, и галочка вставала не на том сообщении. −1 = не из строки
     * (pending/системное). Только в памяти, на провод не идёт (§17).
     */
    val lineIndex: Int = -1,
    /**
